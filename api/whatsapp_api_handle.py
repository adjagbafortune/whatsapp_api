...
class API:
    ...
    def gptResponse(self) -> str:
        ...
        system_content = open("api/assets/training.md").read().format(
            help_message=self.get_all_help_message(), 
            timezone=self.message.timezone, 
            time=datetime.now(pytz.timezone(self.message.timezone)).strftime("%Y-%m-%d %H:%M:%S")
        )
        ...
        messages = [
            ...
            *self.get_previous_messages(),
            {"role": "user", "content": self.message.incoming_text_message},
        ]

        # Appel au service isolé
        response_text = get_ai_response(messages)
        
        # Nettoyage au cas où Gemini renvoie du texte avec des balises markdown de code
